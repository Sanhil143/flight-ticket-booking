...
function compressImages(file,folderPath) {
  ...
  return new Promise((resolve, reject) => {
    const ext = path.extname(file.files.originalname).toLowerCase();
    const outputPath = path.join(folderPath, `${Date.now()}-${file.files.originalname}`);

    const sharpInstance = sharp(file.files.path);
